...
for (let i = 0; i < accountSpecs.length; i++) {
  ...
  fs.writeFileSync(
    path.join(accountsDir, `account_${accountNumber}`),
    account.privateKey,
    'utf8'
  );
  
  fs.writeFileSync(
    path.join(accountsDir, `account_${accountNumber}.pub`),
    account.address,
    'utf8'
  );
  ...
  console.log(`  Private key saved to: accounts/account_${accountNumber}`);
  console.log(`  Public address saved to: accounts/account_${accountNumber}.pub`);
}
