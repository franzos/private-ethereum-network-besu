...
try {
  ...
  if (fs.existsSync(envPath)) {
    ...
    envValues = {
      ...
      currencyDecimals: parseInt(envConfig.CURRENCY_DECIMALS || defaults.currencyDecimals)
    };
  } else {
    console.warn('Warning: .env file not found in the repository root. Using default values.');
  }
} catch (error) {
  console.warn(`Warning: Error reading .env file: ${error.message}. Using default values.`);
}
